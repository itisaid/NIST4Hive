...
public class OracleProcessor {
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.trim().endsWith(";")) {
					...
					if (outLine.startsWith("SELECT")) {
						...
						// for a patch
						String tmpScm;
						if ((tmpScm = TestResultPatch1.needPatch(outLine,
								schema)) != null) {
							if (stmtMap.get(tmpScm) != null) {

								ff.write(("-----------"+seqStr+":"+schema+":"+outLine+"\r\n").getBytes());
								
								// there is a logic bug in the line, same name
								// csv file should be covered, but no effect in
								// fact. the load() invoke must be later.
								String ld = load(stmtMap.get(tmpScm), outDir,
										ff, seqStr);
						...
						while (rs.next()) {
							...
							for (int i = 0; i < rs.getMetaData()
									.getColumnCount(); i++) {
								tmp += rs.getObject((i + 1)) == null ? "NULL"
	...
	public static String load(Statement st, String outDir, FileOutputStream ff,
		...
		Set<String> conflictSet = new HashSet<String>();// ignore table(view)
														// name of uppercase or
														// lowercase
				...
				try {
					...
					while (loadRs.next()) {
						for (int j = 1; j <= loadRs.getMetaData()
							...
							nf.write(o == null ? "\\N".getBytes() : o.toString()
									.getBytes());
							nf.write(",".getBytes());
						}
						nf.write("\n".getBytes());
